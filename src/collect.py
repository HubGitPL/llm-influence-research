...
# --- Status tracking (INFRA-01, INFRA-02) ---

def checkpoint_exists(out_dir: Path, metric: str) -> bool:
    return (out_dir / f"{metric}.done").exists()

def mark_done(out_dir: Path, metric: str):
    (out_dir / f"{metric}.done").touch()


# --- CSV (CMIT-04, PR-02, ISS-04) ---
...
# --- gh API wrappers (INFRA-03, INFRA-04, INFRA-05) ---
...
def run_gh_with_retry(url: str, paginate: bool = False):
    """Wywołaj gh api z retry dla 403/502/504 (INFRA-04, INFRA-05)."""
